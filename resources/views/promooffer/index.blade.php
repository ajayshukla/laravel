    ...
    <div class="col-lg-12 col-xs-12 col-sm-12">
                ...
                <div class="portlet light">
                                <div class="portlet-title">
                                    ...
                                    <div class="actions">
                                        <a href="{{ url('/promooffer/create') }}" class="btn btn-circle btn-info">
                                ...
                                <div class="portlet-body">
                                    <div class="table-container">
                                        <div class="table-actions-wrapper" style="display:block">
                                            <span> </span>
                                            {{ Form::open(array('url' => 'promooffer/filterpromooffer/', 'name' => 'searchbyrole')) }}
                                        ...
                                        <table class="table table-striped table-bordered table-hover table-checkable" id="datatable_products">
                                            <thead>
                                                ...
                                                @else
                                                @foreach($promooffer as $promooffers)
                                                <tr role="row" class="filter">
                                                    <td>
                                                    <label class="mt-checkbox mt-checkbox-single mt-checkbox-outline">
                                                    ...
                                                    <td>{{ $promooffers->id }}</td>
                                                    <td>{{ $promooffers->offertitle }}</td>
                                                    <td>{{ $promooffers->offerref }}</td>
                                                    <td>{{ $promooffers->effectivedate }}</td>
                                                    <td>{{ $promooffers->effectivetime }}</td>
                                                    @if(Auth::user()->role_id==1)
                                                    <td> {{ $promooffers->name }} </td>
                                                    @endif
                                                    <td>
                                                        {{ $promooffers->status }}
                                                    </td>
                                                    <td>
                                                        <div class="margin-bottom-5">
                                                            <a href="{{ route('promooffer.edit', $promooffers->id) }}"><button class="btn btn-sm btn-success filter-submit margin-bottom">Edit</button></a>
                                                        {!! Form::open(['method' => 'DELETE','route' => ['promooffer.destroy', $promooffers->id]]) !!}
                                                           <button onclick="return deletepromooffer(this.value)" class="btn btn-sm btn-success filter-submit margin-bottom">Delete</button>
        												{!! Form::close() !!}  
                                                        <a href="{{ URL::to('promooffer/' . $promooffers->id) }}"><button class="btn btn-sm btn-success filter-submit margin-bottom">View</button></a>  
                                                        </div>
